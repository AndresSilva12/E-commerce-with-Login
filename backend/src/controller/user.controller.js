...
export const createUser = async (req, res, next) => {
    try {
        ...
        console.log('debug:',passwordHashed)
...
export const dashboardProtected = (req, res, next) => {
    ...
    if (!accessToken) return res.status(403).json({error: "Acceso denegado. Debes iniciar sesion primero"})
    try{
        const data = jwt.verify(accessToken, "123")
        return res.status(200).json(`Hola ${data.username}. Estamos accediendo a la dashboard...`)
    }
    catch (error) {
        console.log(error)
        return res.status(403).json({error: "Acceso denegado. Debes iniciar sesion primero"})
    }
}
